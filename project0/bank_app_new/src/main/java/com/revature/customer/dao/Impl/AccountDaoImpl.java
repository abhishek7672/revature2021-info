...
import com.revature.main.Bankmain;
import com.revature.menu.TransactionMenu;
...
public class AccountDaoImpl implements AccountDao
{
	...
	public ArrayList<Account> showAccounts() throws SQLException {
		...
		//String email = Bankmain.getCurrentCustomer().getEmail();
		try (Connection connection = Util.getconnection()) {
			String sql = "select * from account where email  =?";
			PreparedStatement statement = connection.prepareStatement(sql);
			statement.setString(1, CustomerDaoImp.currentcustomerid);
	...
	public Account currentAccount() throws SQLException {
		...
		try (Connection connection = Util.getconnection()) {
			...
			ResultSet resultSet = statement.executeQuery();
			while (resultSet.next()) {
				currentAccountId = resultSet.getInt("accountno");
				int accNumber = resultSet.getInt("accountno");
	...
	public Account transferAccount() throws SQLException {
		...
		try (Connection connection = Util.getconnection()) {
			...
			ResultSet resultSet = statement.executeQuery();
			while (resultSet.next()) {
				transferAccountId = resultSet.getInt("accountno");
				int accNumber = resultSet.getInt("accountno");
